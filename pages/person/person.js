...
Page({
  ...
  start: function () {
    ...
    app.questUrl("jeecg-boot/infrastructure/building/findPersonSize", 'post', token, data).then(function (res) {
      console.log(res)
      if (res.data) {
        that.setData({
  ...
  tovideoplay: function (e) {
    ...
    wx.navigateTo({
      url: '../videoplay/videoplay?val=' + item.value + '&name=' + item.name + '&title=' + that.data.title + '&idx=' + idx,
    })
  },
  /**
   * 生命周期函数--监听页面初次渲染完成
